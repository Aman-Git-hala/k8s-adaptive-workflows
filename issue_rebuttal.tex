...
On April 16, 2026, three issues were opened on my GitHub repository by the user \texttt{anshika289}. 
...
This distinction matters a lot. A ``MUST'' requirement that is unmet is a real problem. But a feature that the SRS never mentions is not a ``bug''.
...
\hline
\#4 ML Prediction Visibility & Usability (Bug) & No SRS requirement exists for this & Visible via \texttt{kubectl}, logs, dashboard & Not mentioned in the SRS \\
\hline
\end{longtable}
...
\begin{longtable}{
  ...
  >{\raggedright\ttfamily\small\arraybackslash}p{5.5cm}
  >{\raggedright\arraybackslash}p{4cm}}
\toprule
\textbf{What They Claim is Missing} & \textnormal{\textbf{File}} & \textbf{Lines} \\
\midrule
\endfirsthead
...
internal/optimizer/greedy.go &
L48--63 (usage sum), L90--105 (hard gate) \\
\midrule

Resource usage tracking &
internal/controller/ adaptiveworkflow\_controller.go &
L249: \textnormal{\texttt{computeResourceUsage()}} \\
\midrule

Resource usage in CRD &
api/v1/adaptiveworkflow\_types.go &
L100: \textnormal{\texttt{CurrentResourceUsage}} field \\
\midrule

Dependency checking (cycle safety) &
internal/optimizer/greedy.go &
L72--83 (all deps must be Succeeded) \\
\midrule

ML predictions in status &
internal/controller/ adaptiveworkflow\_controller.go &
L235--237: \textnormal{\texttt{AllocatedResources}} per task \\
\midrule

Inference engine predictions &
inference-engine/server.py &
\textnormal{\texttt{Predict()}} RPC handler \\
\midrule

Metrics to StateDB &
internal/controller/ adaptiveworkflow\_controller.go &
L312--340: \textnormal{\texttt{RecordExecution()}} \\
\bottomrule
\end{longtable}
