...
% ── Tip/Warning boxes ────────────────────────────────────────────────────────
\newtcolorbox{srsbox}{colback=blue!5,colframe=codeblue,title=\textbf{What Our SRS Says},fonttitle=\bfseries}
\newtcolorbox{codebox}{colback=green!3,colframe=green,title=\textbf{What Our Code Actually Does},fonttitle=\bfseries}
...
{\large\textbf{Asst. Prof. Bhawna Rawat}}\\[0.2cm]
...
On April 16, 2026, three issues were opened on our GitHub repository by the user \texttt{anshika289}. All three were filed within a short time window and follow the exact same template format.

After going through every single claim in these issues and comparing them line-by-line against our actual source code and our SRS document, we are confident that:

\begin{itemize}
  \item \textbf{Issue \#3} (marked ``Critical'') is \textbf{factually wrong}. The feature they say is missing is literally written in our code with clear comments. They did not read our source code.
  \item \textbf{Issue \#2} has a \textbf{small valid point} buried under exaggerated claims. The system handles cycles safely, but we could show a better error message. That said, calling it a ``bug'' is a stretch --- our system never crashes or freezes from cycles.
  \item \textbf{Issue \#4} describes a \textbf{feature that is not even in our SRS}. The data they want is already available through standard Kubernetes commands. Their framing makes it sound like a defect, but it is at best a nice-to-have improvement.
\end{itemize}

\subsection{A Note On Issue Quality}

Before we begin the detailed rebuttal, we want to point out some serious quality concerns with these issues:
...
Our SRS document follows \textbf{IEEE 830-1998} conventions. The exact wording from SRS Section 1.2:
...
\textit{We start with Issue \#3 because it is marked ``Critical'' and is the most egregiously wrong.}
...
\subsection{What Our Code Actually Does}
...
This is not a bug. This is \textbf{the entire point of our system}. When all the available resource budget is used up by running tasks, the optimizer waits for some tasks to finish before starting new ones. The ``unknown time'' is just the time it takes for pods to complete their work. That is how every single DAG scheduler in existence works --- Argo Workflows, Apache Airflow, Tekton, all of them.

Our SRS calls this ``Bin Packing'' (Section 1.4): fitting as many tasks as possible onto available hardware without crashing the nodes. That is exactly what the code does.
...
We will be honest here. Our SRS does say ``MUST validate'' for cycle detection. So on paper, this is a valid requirement to bring up. \textbf{However}, the issue dramatically exaggerates the impact and makes claims that are physically impossible given our architecture.

\subsection{Why ``Infinite Loops or Scheduler Freeze'' Cannot Happen}

Let us walk through what actually happens if someone submits a cycle like A $\to$ B $\to$ C $\to$ A.

Our optimizer (\texttt{internal/optimizer/greedy.go}, lines 72--83) only schedules a task if \textbf{ALL of its dependencies have succeeded}:
...
\subsection{What IS Missing (And We Acknowledge It)}
...
We do not have that validation step. The system handles cycles \textit{safely} (nothing breaks), but it does not handle them \textit{helpfully} (the user gets no clear message about why nothing is happening).
...
\begin{enumerate}
  \item They claim ``may cause infinite loops or scheduler freeze.'' This is \textbf{impossible}. Our reconcile loop is stateless and runs on a fixed 10-second timer (controller line 264). It cannot loop infinitely or freeze.
...
\begin{srsbox}
Here are the ML-related requirements from our SRS (Section 4.3.3, Page 14):
...
They say users should be able to see ``Confidence/accuracy'' for each prediction. Let us be real here:
...
\begin{itemize}
  ...
  \item Even if we wanted to add it, our SRS does not require it.
\end{itemize}
...
\hline
\textbf{Issue} & \textbf{Claimed Severity} & \textbf{SRS Requirement} & \textbf{Met?} & \textbf{Our Assessment} \\
\hline
...
For comparison, a well-written issue on a codebase like ours would include:
\begin{enumerate}
...
None of the three issues meet any of these standards. They read like they were written based on a quick scan of our README, not from actually running the code or reading the implementation.
...
\vspace{0.5cm}
\textit{This document was prepared by the development team in response to issues filed on our repository. We are happy to demonstrate any of the above code paths live during the evaluation.}
